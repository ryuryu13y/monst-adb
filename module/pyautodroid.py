...
def OriginalBreak(device, temp, threshold=0.97, trim=(0, 0, 0, 0)):
    file_id = device.replace(':', '_')
    img = cv2.imread('C:'+SHARED_DIR+'/ImageShare/screen'+file_id+'.png', 1)
...
def swipe2(device, src, duration=500):
    call(ADB_PATH+' -s '+device+' shell input swipe '+str(src[0])+' '+str(src[1])+' '+str(src[0])+' '+str(src[1])+' '+str(duration))
    return
...
def get_screen(device, path='/mnt/shared/ImageShare/'):
    path=("C:/Users/jesc1/Nox_share/ImageShare/")
    file_id = device.replace(':', '_')
